...
namespace Bumbo.Logic.PayCheck
{
    public class PayCheckLogic
    {
        ...
        {
            if (!workedShift.EndTime.HasValue)
            {
                throw new ArgumentNullException(nameof(workedShift.EndTime), "Bonus can't be calculated for a shift that hasn't ended yet. EndTime cannot be null");
            }
            ...
            else if (workedShift.Shift.Date.DayOfWeek == DayOfWeek.Saturday)
            {
                ...
                var startTime = workedShift.StartTime.Hours > 6
                    ? workedShift.StartTime
                    : new TimeSpan(6, 0, 0);

                var endTime = workedShift.EndTime.Value.Hours > 18
                    ? new TimeSpan(18, 0, 0)
        ...
        public TimeSpan BonusTimeBetween00And06(WorkedShift workedShift)
        {
            if (!workedShift.EndTime.HasValue)
            {
                throw new ArgumentNullException(nameof(workedShift.EndTime));
            }
            ...
            if (!validTimeFrame)
            {
                validTimeFrame = workedShift.StartTime.Hours < 6;
            }
